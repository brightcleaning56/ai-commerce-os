...
/**
 * POST /api/transactions/[id]/release  — release escrow to supplier.
 *
 * Operator-gated (or auto-triggered by the 7-day timeout cron — future).
 *
 * Transitions:
 *   delivered → released → completed (immediately advanced together)
 *
...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    const released = await transitionTransaction({
      ...
      to: "released",
      actor: "operator",
      detail: "Escrow released — supplier payout authorized",
      patch: {
