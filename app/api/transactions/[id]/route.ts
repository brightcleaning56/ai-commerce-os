 ...
 *
 * Two access patterns:
 *   1. Operator (Authorization: Bearer ADMIN_TOKEN or aicos_admin cookie) â€” full data
 *   2. Buyer (?t=<shareToken>) â€” full data + signs the public viewer flow
...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  // Auto-release window â€” exposed so the buyer page can render the
  ...
  if (token && token === txn.shareToken) {
    // Public buyer access â€” check expiry
