...
async function findBuyerEmail(buyerId: string, buyerCompany: string): Promise<{ email: string | null; source: "discovered" | "static" | "business" | null }> {
  // Business-directory id ("biz_*") — drafts generated by
  // /api/admin/businesses/draft-outreach use this prefix. Look up via
  // the businesses store and ALSO honor the suppression gate here as a
  // defense-in-depth check (in case a record was DNC'd between draft
  // and send).
  if (buyerId.startsWith("biz_")) {
    const biz = await store.getBusiness(buyerId);
    if (biz) {
      // Suppression: never send to a do_not_contact business even if
      // a stale draft references it.
      if (biz.status === "do_not_contact" || biz.doNotContact || biz.optedOutAt) {
        return { email: null, source: "business" };
      }
      return { email: biz.email ?? null, source: "business" };
    }
