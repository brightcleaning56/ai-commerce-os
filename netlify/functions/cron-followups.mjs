...
export default async () => {
  ...
  try {
    const res = await fetch(`${baseUrl}/api/cron/followups`, { headers });
    const body = await res.text();
    const elapsed = Date.now() - start;
    if (!res.ok) {
      console.warn(`[cron-followups] HTTP ${res.status} after ${elapsed}ms: ${body.slice(0, 200)}`);
      return new Response(
        JSON.stringify({ ok: false, status: res.status, elapsed }),
        { status: 200, headers: { "Content-Type": "application/json" } },
      );
    }
    console.log(`[cron-followups] ok in ${elapsed}ms`);
    return new Response(
    ...
    console.error(`[cron-followups] failed after ${elapsed}ms:`, msg);
    return new Response(
